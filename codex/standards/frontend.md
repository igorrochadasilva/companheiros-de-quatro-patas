...
- Use the feature-based structure:
  - `src/features/<feature>/hooks` for domain hooks
  - `src/features/<feature>/services` for API calls
  - `src/features/<feature>/components` for feature-specific components
- Route components stay in `src/app/(public|admin)/...` and import logic from `features/*`.
